...
In the following code chunks, we will be working with the data frame `McDonalds_FullData`. This data set was adapted from Kaggle dataset: [Nutrition Facts for McDonald's Menu](https://www.kaggle.com/datasets/mcdonalds/nutrition-facts). It contains the nutritional value items on McDonald's menu. 
...
Click on the "Solution" button to see code with all the adjustments above. 
...
Looking the scatter plot above, we see that there is one outlier. In this section, we will highlight that outlier and add a label. We will first create a data frame with just the outlier and use `McDonalds_FullData` and `outlier` in `ggplot()`. Compare the code chunk below with the code chunk above -- what new lines of code do you see? What do you think their functions are?
